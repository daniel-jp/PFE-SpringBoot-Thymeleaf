...
public interface FormRepository extends JpaRepository<Form, String> {
	  ...
	  Long countFormByStatus(Status todo);
	    ...
	    @Transactional
	    @Modifying
	    @Query("UPDATE Form SET position = position + 1 WHERE position >= :position AND status = :status AND id <> :id")
	    ...
	    @Transactional
	    @Modifying
	    @Query("UPDATE Form SET position = position + 1 WHERE position >= :newPosition AND position < :oldPosition AND status = :status AND id <> :id")

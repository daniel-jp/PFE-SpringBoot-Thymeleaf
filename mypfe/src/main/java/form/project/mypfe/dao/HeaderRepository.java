...
public interface HeaderRepository extends JpaRepository<HeaderField, String> {
	  ...
	  Long countFormByStatus(Status todo);
	    ...
	    @Transactional
	    @Modifying
	    @Query("UPDATE HeaderField SET position = position + 1 WHERE position >= :position AND status = :status AND id <> :id")
	    ...
	    @Transactional
	    @Modifying
	    @Query("UPDATE HeaderField SET position = position + 1 WHERE position >= :newPosition AND position < :oldPosition AND status = :status AND id <> :id")
